...
ggplot(gapminder2007, mapping=aes(x=continent, y=gdpPercap)) + 
  geom_boxplot()
```
Regresson tables use the first category as the intercept. that's illustrated in the following two regresson tables. The estimated values for any given category are the `intercept + estimate`. Mathematically, this works by applying an indicator function to each pair of (coefficient, value).
```{r}
